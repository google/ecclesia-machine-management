...
TEST_F(SystemEventVisitorTest, BootNumberFound) {
  EXPECT_CALL(*reader_, ReadEvent)
      .WillOnce(Return(SystemEventRecord{.record = MachineCheck{}}))
      .WillOnce(Return(SystemEventRecord{.record = MachineCheck{.boot = 233}}))
      .WillOnce(Return(SystemEventRecord{.record = MachineCheck{}}))
      .WillOnce([&]() {
        last_event_logged_.Notify();
        return absl::nullopt;
  ...
  logger.Visit(&visitor);

  ASSERT_TRUE(visitor.GetBootNumber().has_value());
  ASSERT_EQ(visitor.GetBootNumber().value(), 233);
}

TEST_F(SystemEventVisitorTest, MultipleBootNumbersFound) {
  EXPECT_CALL(*reader_, ReadEvent)
      .WillOnce(Return(SystemEventRecord{.record = MachineCheck{.boot = 233}}))
      .WillOnce(Return(SystemEventRecord{.record = MachineCheck{}}))
      .WillOnce(Return(SystemEventRecord{.record = MachineCheck{.boot = 0}}))
      .WillOnce([&]() {
        last_event_logged_.Notify();
        return absl::nullopt;
      })
  ...
  ASSERT_TRUE(visitor.GetBootNumber().has_value());
  ASSERT_EQ(visitor.GetBootNumber().value(), 0);
}

TEST_F(SystemEventVisitorTest, BootNumberNotFound) {
  EXPECT_CALL(*reader_, ReadEvent)
      .WillOnce(Return(SystemEventRecord{.record = MachineCheck{}}))
      .WillOnce([&]() {
        last_event_logged_.Notify();
  ...
  ASSERT_FALSE(visitor.GetBootNumber().has_value());
}
