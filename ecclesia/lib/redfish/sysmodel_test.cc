...
TEST_F(SysmodelTest, GetResourceMemoryExpands) {
  int expanded_request_count = 0;
  InitServer("topology_v2_testing/mockup.shar");
  // Store original json
  mockup_server_->AddHttpGetHandler(
      ...
      [&](ServerRequestInterface *req) {
        expanded_request_count++;
        SendJsonHttpResponse(req, R"json()json");
      });
  sysmodel_->QueryAllResources<ResourceMemory>(
      [&](std::unique_ptr<RedfishObject>) -> RedfishIterReturnValue {
        return RedfishIterReturnValue::kStop;
      });
  EXPECT_EQ(expanded_request_count, 1);
...
TEST_F(SysmodelTest, GetSensorsExpands) {
  int expand_thermal_count = 0;
  int expand_power_count = 0;
  int expand_sensors_count = 0;
  InitServer("topology_v2_testing/mockup.shar");
  // Store original json
  mockup_server_->AddHttpGetHandler(
      "/redfish/v1/Chassis/child2/Thermal?$expand=.($levels=1)",
      [&](ServerRequestInterface *req) {
        expand_thermal_count++;
        SendJsonHttpResponse(req, "");
      });
  mockup_server_->AddHttpGetHandler(
      "/redfish/v1/Chassis/child2/Power?$expand=.($levels=1)",
      [&](ServerRequestInterface *req) {
        expand_power_count++;
        SendJsonHttpResponse(req, "");
      });
  mockup_server_->AddHttpGetHandler(
      "/redfish/v1/Chassis/child2/Sensors?$expand=.($levels=1)",
      [&](ServerRequestInterface *req) {
        expand_sensors_count++;
        SendJsonHttpResponse(req, "");
      });

  sysmodel_->QueryAllResources<ResourceTemperature>(
      [&](std::unique_ptr<RedfishObject>) -> RedfishIterReturnValue {
        return RedfishIterReturnValue::kStop;
      });
  EXPECT_EQ(expand_thermal_count, 1);

  sysmodel_->QueryAllResources<ResourceVoltage>(
      [&](std::unique_ptr<RedfishObject>) -> RedfishIterReturnValue {
        return RedfishIterReturnValue::kStop;
      });
  EXPECT_EQ(expand_power_count, 1);

  sysmodel_->QueryAllResources<ResourceFan>(
      [&](std::unique_ptr<RedfishObject>) -> RedfishIterReturnValue {
        return RedfishIterReturnValue::kStop;
      });
  EXPECT_EQ(expand_thermal_count, 2);

  sysmodel_->QueryAllResources<ResourceSensor>(
      [&](std::unique_ptr<RedfishObject>) -> RedfishIterReturnValue {
        return RedfishIterReturnValue::kStop;
      });
  EXPECT_EQ(expand_sensors_count, 1);
}
