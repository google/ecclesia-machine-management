...
MmioAccess::MmioAccess(AddressRange mmap_addr_range,
                       absl::string_view physical_mem_device)
    : MmioAccess(mmap_addr_range.Empty() ? 0
                                         : mmap_addr_range.LastAddress() -
                                               mmap_addr_range.FirstAddress(),
                 mmap_addr_range.FirstAddress(), physical_mem_device) {}

MmioAccess::MmioAccess(size_t size, uint64_t first_address,
                       absl::string_view physical_mem_device)
    : size_(size),
...
absl::StatusOr<uint8_t> MmioAccess::Read8(OffsetType offset) const {
...
absl::Status MmioAccess::Write8(OffsetType offset, uint8_t value) {
  ECCLESIA_RETURN_IF_ERROR(mmio_.status());
  ECCLESIA_RETURN_IF_ERROR(
...
absl::StatusOr<uint16_t> MmioAccess::Read16(OffsetType offset) const {
  ECCLESIA_RETURN_IF_ERROR(mmio_.status());
...
absl::Status MmioAccess::Write16(OffsetType offset, uint16_t value) {
  ECCLESIA_RETURN_IF_ERROR(mmio_.status());
  uint8_t data[sizeof(uint16_t)];
...
absl::StatusOr<uint32_t> MmioAccess::Read32(OffsetType offset) const {
  ECCLESIA_RETURN_IF_ERROR(mmio_.status());
...
absl::Status MmioAccess::Write32(OffsetType offset, uint32_t value) {
  ECCLESIA_RETURN_IF_ERROR(mmio_.status());
  uint8_t data[sizeof(uint32_t)];
